(function(){
  ...
  function updateFull(){const a=list();if(!a.length)return;captureVisibleActuals();save();window.__agendaTrainingActuals={fillPrevious:function(log){try{if(!activeSession)return;ensureActuals();const data=activeSession.actuals[index];if(!data)return;const parseSeries=(value)=>{const out=[];const text=String(value??'').trim();if(!text)return out;text.split('·').forEach((part)=>{const m=part.match(/S\s*(\d+)\s*:\s*(.*)/i);if(m){const n=Number(m[1])-1;if(n>=0)out[n]=m[2].trim()}else if(part.trim())out.push(part.trim())});if(out.length===1 && /\//.test(text)){text.split('/').forEach((v,i)=>{if(v.trim())out[i]=v.trim()})};return out};const reps=parseSeries(log?.reps);const weights=parseSeries(log?.weight);for(let i=0;i<data.reps.length;i++){if(reps[i]!==undefined&&reps[i]!==''&&reps[i]!=='-')data.reps[i]=String(reps[i]);if(weights[i]!==undefined&&weights[i]!==''&&weights[i]!=='-')data.weight[i]=String(weights[i]);else if(weights.length===1&&weights[0]&&weights[0]!=='-')data.weight[i]=String(weights[0]);}save();renderRepTracker();setTimeout(()=>{const host=q('v47SetList');if(!host)return;const rows=[...host.querySelectorAll('.v48-set-row,.v47-set-row')];rows.forEach((row,i)=>{const inputs=[...row.querySelectorAll('input')];const d=activeSession?.actuals?.[index];if(!d)return;if(inputs[0]&&d.reps[i]!==undefined)inputs[0].value=d.reps[i]??'';if(inputs[1]&&d.weight[i]!==undefined)inputs[1].value=d.weight[i]??'';if(inputs[2]&&d.rir[i]!==undefined)inputs[2].value=d.rir[i]??'';});updateRepStatus()},0)}catch(e){console.warn('No se pudieron copiar los datos anteriores:',e)}}};index=Math.max(0,Math.min(index,a.length-1));const ex=a[index];q('v28WorkoutDayLabel').textContent=`${DAYS[workoutDay]} · ${a.length} ejercicios`;q('v28StepLabel').textContent=`Ejercicio ${index+1} de ${a.length}`;q('v28CurrentName').textContent=ex.exercise||'Ejercicio';const chips=q('v28CurrentChips');chips.innerHTML='';[['Series',ex.sets],['Reps',ex.reps],['RIR',ex.rir],['Descanso',ex.rest_seconds?`${ex.rest_seconds}s`:null],['Peso',ex.weight],['Tempo',ex.tempo]].forEach(([k,v])=>{if(v===null||v===undefined||v==='')return;const c=document.createElement('span');c.className='v28-chip';c.textContent=`${k}: ${v}`;chips.appendChild(c)});q('v28CurrentNote').textContent=ex.notes||'';q('v28CurrentNote').style.display=ex.notes?'block':'none';q('v28PrevBtn').disabled=index===0;q('v28NextBtn').disabled=index===a.length-1;q('v28ProgressBar').style.width=`${Math.round((index+1)/a.length*100)}%`;const s=q('v28SessionList');s.innerHTML='';a.forEach((e,i)=>{const p=document.createElement('div');p.className='v28-session-pill'+(i===index?' active':'');const done=activeSession?.loggedIndexes?.includes(i);p.textContent=`${i+1}. ${e.exercise||'Ejercicio'}${done?' ✓':''}`;p.onclick=()=>{index=i;save();updateFull()};s.appendChild(p)});q('v28TimerDisplay').textContent=fmt(timerSeconds);q('v28SessionClock').textContent=fmt(sessionSeconds);updateMini();renderRepTracker()}
