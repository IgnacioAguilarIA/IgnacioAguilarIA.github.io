/* Agenda FICH · V89 · Weather module
...
(function(){
  ...
  const CFG={
    ...
    cacheKey:'agendaWeatherV89',
    coordsKey:'agendaWeatherCoordsV89',
    ttlMs:30*60*1000,
  ...
  function isOutdoorTask(t){
    const text=String([t?.title,t?.description,t?.notes].filter(Boolean).join(' ')).toLowerCase();
    if(!text)return false;
    return /(aire libre|al aire libre|afuera|exterior|parque|plaza|correr|running|trote|bicic|bici|f[uú]tbol|futbol|tenis|padel|caminar|caminata|senderismo|paseo|salir|salida|pileta|nataci[oó]n|jard[ií]n|patio|cancha|skate|skateboard|ciclismo)/i.test(text);
  }
  function taskMinutes(t){return (Number(t?.hour)||0)*60+(Number(t?.minute)||0)}
  function todayTasks(){
    try{
      const idx=typeof getTodayIndex==='function'?getTodayIndex():Number(selectedDay)||0;
      return Array.isArray(tasks)?tasks.filter(t=>Number(t.day)===idx&&!t.completed):[];
    }catch(_){return []}
  }
  function weatherTaskRisk(data){
    ...
    const weather=Number(d.weather_code?.[0]);
    const storm=[95,96,99].includes(weather);
    return todayTasks().filter(isOutdoorTask).map(t=>{
      const severe=storm||rain>=70||sum>=5;
      const moderate=rain>=40||sum>0;
      return {...t,risk:severe?'high':moderate?'medium':'low',rain,sum,storm};
    }).filter(x=>x.risk!=='low');
  }
  function renderWeatherTaskAlerts(data){
    const list=weatherTaskRisk(data);
    let host=document.getElementById('v89WeatherTaskAlerts');
    const agenda=document.querySelector('.schedule-block.v32-agenda-pane');
    if(!agenda||!currentUser)return;
    if(!host){host=document.createElement('section');host.id='v89WeatherTaskAlerts';host.className='v89-weather-task-alerts';agenda.parentElement?.insertBefore(host,agenda);}
    if(!list.length){host.style.display='none';return;}
    host.style.display='block';
    const label=list.some(x=>x.risk==='high')?'⚠️ Actividades al aire libre con clima desfavorable':'🌦️ Actividades al aire libre con posible lluvia';
    const first=list[0];
    const detail=first.risk==='high'
      ? `Hoy hay ${first.rain}% de probabilidad de precipitación${first.sum>0?` y ${first.sum.toFixed(1)} mm estimados`:''}.`
      : `Hoy hay ${first.rain}% de probabilidad de precipitación.`;
    host.innerHTML=`<div class="v89-weather-task-head"><div><strong>${label}</strong><span>${detail} Revisá estos horarios.</span></div><button type="button" id="v89WeatherTaskCalendar">Ver calendario</button></div><div class="v89-weather-task-list">${list.slice(0,5).map(t=>`<div class="v89-weather-task-row"><span class="v89-weather-task-icon">${t.risk==='high'?'🌧️':'🌦️'}</span><div><strong>${esc(t.title||'Actividad')}</strong><small>${String(t.hour??0).padStart(2,'0')}:${String(t.minute??0).padStart(2,'0')} · ${t.description?esc(t.description):'Actividad al aire libre'}</small></div><span class="v89-weather-task-badge">${t.rain}%</span></div>`).join('')}</div>`;
    document.getElementById('v89WeatherTaskCalendar')?.addEventListener('click',()=>{
      try{v32SetSection?.('calendar')}catch{}
      setTimeout(()=>document.getElementById('calendarPanel')?.scrollIntoView({behavior:'smooth',block:'start'}),100)
    });
  }
