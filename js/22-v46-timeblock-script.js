(function(){
  ...
  const DUR_KEY=()=>`agendaV46:${currentUser?.id||'guest'}:durations`;
  const MOVE_KEY=()=>`agendaV46:${currentUser?.id||'guest'}:moves`;
  let viewDay=0;
  function safeParse(k,f){try{const r=localStorage.getItem(k());return r?JSON.parse(r):f}catch{return f}}
  function durations(){const cur=safeParse(DUR_KEY,{}); if(!Object.keys(cur).length){try{const old=localStorage.getItem(`agendaV45:${currentUser?.id||'guest'}:durations`);if(old)return JSON.parse(old)}catch(_){}} return cur}
  function dur(t){const m=durations();return Math.max(5,Number(t?.duration_minutes||m[String(t?.id)]||30)||30)}
  function setDur(t,n){if(!t?.id)return;const m=durations();m[String(t.id)]=Math.max(5,Math.min(240,Number(n)||30));try{localStorage.setItem(DUR_KEY(),JSON.stringify(m))}catch(_){} }
  function moves(){return safeParse(MOVE_KEY,[])}
  function saveMoves(a){try{localStorage.setItem(MOVE_KEY(),JSON.stringify(a.slice(-100)))}catch(_){} updateSyncBadge()}
  function queueMove(t,day,hour,minute){const a=moves().filter(x=>String(x.id)!==String(t.id));a.push({id:String(t.id),day,hour,minute,queuedAt:new Date().toISOString()});saveMoves(a)}
  function updateSyncBadge(){const b=q('v46MoveSync');if(!b)return;const n=moves().length;if(!navigator.onLine){b.textContent=`📴 ${n} cambio(s) offline`;b.className='v46-sync pending';}else if(n){b.textContent=`☁️ ${n} por sincronizar`;b.className='v46-sync pending';}else{b.textContent='☁️ Sincronizado';b.className='v46-sync ok';}}
  async function flushMoves(){if(!navigator.onLine||!currentUser)return;const a=moves();if(!a.length){updateSyncBadge();return}const remain=[];for(const m of a){try{const {error}=await sb.from('tasks').update({day:m.day,hour:m.hour,minute:m.minute}).eq('id',m.id);if(error)throw error;}catch(e){remain.push(m)}}saveMoves(remain);if(!remain.length){try{await loadTasks();renderSchedule?.();renderDashboard?.();renderConflicts?.();}catch(_){} }render();}
  function dayDate(i){const d=typeof localDateForDay==='function'?localDateForDay(i):new Date();return d}
  function dayName(i){return (typeof DAYS!=='undefined'?DAYS[i]:['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'][i])}
  function pRank(t){const p=typeof getPriority==='function'?getPriority(t):String(t?.priority||'normal');return p==='urgent'?0:p==='high'?1:p==='low'?3:2}
