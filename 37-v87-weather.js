...
(function(){
  'use strict';
  const CFG={
    cacheKey:'agendaWeatherV87',
    coordsKey:'agendaWeatherCoordsV87',
    ttlMs:30*60*1000,
  ...
  function requestLocation(){
    ...
    navigator.geolocation.getCurrentPosition(
      pos=>{saveCoords({lat:Number(pos.coords.latitude),lon:Number(pos.coords.longitude)});const r=$('v87WeatherRefreshBtn');if(r)r.disabled=!navigator.onLine;loadWeather(true)},
  ...
  function init(){
    ...
    const c=getCoords();
    if(c){$('v87WeatherLocationLabel').textContent=`📍 ${Number(c.lat).toFixed(3)}, ${Number(c.lon).toFixed(3)}`;loadWeather(false)}
